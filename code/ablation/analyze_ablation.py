...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generic immutable-campaign ablation analysis."
    )
    parser.add_argument("--campaign", required=True)
    parser.add_argument(
        "--evaluation-split", choices=("formal", "external"), default="formal"
    )
    parser.add_argument(
        "--output-root", type=Path, default=PACKAGE_ROOT / "results" / "ablation"
    )
...
def metric(
    section: dict[str, Any], mode: str, name: str, statistic: str = "mean"
) -> float | None:
...
def main() -> int:
    ...
    for case_dir in sorted(
        (run_root / "cases").iterdir() if (run_root / "cases").is_dir() else []
    ):
        ...
        if not isinstance(section, dict):
            ...
            continue
        case_items.append(
            {"case_dir": case_dir, "summary": summary, "section": section}
        )
    ...
    for item in case_items:
        ...
        for mode in ("zero", "real"):
            if mode not in section:
                continue
            per_record = load_per_record(
                item["case_dir"] / f"{args.evaluation_split}_per_record.csv", mode
            )
            ranking.append(
                {
                    ...
                    "peak_relative_error_pct_median": metric(
                        section, mode, "peak_relative_error_pct", "median"
                    ),
                    "pier_top_rmse_physical_mean": metric(
                        section, mode, "pier_top_rmse_physical"
                    ),
    ...
    ranking.sort(
        key=lambda row: (
            float(row["rmse_physical_mean"]),
            -float(row["correlation_mean"]),
        )
    )
    ...
    for group, pair in groups.items():
        ...
        b0_rows = load_per_record(
            b0["case_dir"] / f"{args.evaluation_split}_per_record.csv", "zero"
        )
        b1_rows = load_per_record(
            b1["case_dir"] / f"{args.evaluation_split}_per_record.csv", "real"
        )
        ...
        improvements = [
            (
                as_float(b0_rows[name], "rmse_physical")
                - as_float(b1_rows[name], "rmse_physical")
            )
            / max(as_float(b0_rows[name], "rmse_physical"), 1.0e-12)
            * 100.0
            for name in common
        ]
        paired.append(
            {
                ...
                "b0_rmse_mean": sum(
                    as_float(b0_rows[name], "rmse_physical") for name in common
                )
                / len(common),
                "b1_rmse_mean": sum(
                    as_float(b1_rows[name], "rmse_physical") for name in common
                )
                / len(common),
                "b0_correlation_mean": sum(
                    as_float(b0_rows[name], "correlation") for name in common
                )
                / len(common),
                "b1_correlation_mean": sum(
                    as_float(b1_rows[name], "correlation") for name in common
                )
                / len(common),
                "b1_rmse_improvement_pct_mean": sum(improvements) / len(improvements),
                "b1_win_rate_pct": sum(value > 0.0 for value in improvements)
                / len(improvements)
                * 100.0,
    ...
    paired.sort(
        key=lambda row: (
            -float(row["b1_rmse_improvement_pct_mean"]),
            float(row["b1_rmse_mean"]),
        )
    )
    ...
    (run_root / "analysis_summary.json").write_text(
